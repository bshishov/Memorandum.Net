...
{
    public class Rfc2898DeriveBytes_sha256 : DeriveBytes
    {
        const int BlockSize = 32;
        uint block;
        byte[] buffer;
        int endIndex;
        readonly HMACSHA256 hmacsha1;
        uint iterations;
        byte[] salt;
        int startIndex;

        public Rfc2898DeriveBytes_sha256(string password, int saltSize)
            : this(password, saltSize, 1000)
        {
        }

        public Rfc2898DeriveBytes_sha256(string password, byte[] salt)
            : this(password, salt, 1000)
        {
        }

        public Rfc2898DeriveBytes_sha256(string password, int saltSize, int iterations)
        {
            if (saltSize < 0)
            {
            ...
            hmacsha1 = new HMACSHA256(new UTF8Encoding(false).GetBytes(password));
            Initialize();
        }

        public Rfc2898DeriveBytes_sha256(string password, byte[] salt, int iterations)
            : this(new UTF8Encoding(false).GetBytes(password), salt, iterations)
        {
        }

        public Rfc2898DeriveBytes_sha256(byte[] password, byte[] salt, int iterations)
        {
            Salt = salt;
            IterationCount = iterations;
            hmacsha1 = new HMACSHA256(password);
            Initialize();
        }
